(function (window, undefined) {
     ...
     * 喜欢
     * @param   {Object} oConf
     *  @param  {String} oConf.entityId 对象id
     *  @param  {String} oConf.entityType 对象类型
    ...
    function fLike(oConf) {
        ...
        that.post({
            url: '/like',
            data: {entityId: oConf.entityId,entityType: oConf.entityType},
            call: oConf.call,
            error: oConf.error,
            always: oConf.always
        });
    ...
    function fPost(oConf) {
        ...
        $.ajax({
            ...
            if (oResult.code === 999) {
                // 未登录
                alert('未登录');
                window.location.href = '/forLogin?next=' + window.encodeURI(window.location.href);
                return;
            }
