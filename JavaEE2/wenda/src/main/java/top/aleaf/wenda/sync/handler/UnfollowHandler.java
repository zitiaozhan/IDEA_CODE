...
public class UnfollowHandler implements EventHandler {
    ...
    public void doHandle(EventModel eventModel) {
        try {
            ...
            if (eventModel.getEntityType().equals(EntityType.ENTITY_USER.getValue())) {
                message.setContent(String.format("用户[%s]取消了对你的关注", actor.getName()));
                ...
                message.setContent(String.format("用户[%s]取消关注了你发布的问题【%s】", actor.getName(), question.getTitle()));
            }
            ...
            LOGGER.error("用户取消关注事件处理异常：" + e.getMessage());
            e.printStackTrace();
        }
    }
